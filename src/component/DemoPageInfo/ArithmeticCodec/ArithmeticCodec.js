...
class ArithmeticCodec extends Component {
  ...
  FrequencyInitiation = () => {
    ...
    this.setState(
      { lowFreq: lowFreq, cumulativeFreqCheck: true },
      this.highFrequencyInitiation
    );
  };
  ...
  EncoderWithUI = () => {
    ...
    if (this.state.lettersEncodedSoFar === 0) {
      ...
      this.setState({
        encodedMsgLength: msgLength,
        messageToBeEncoded: messageToBeEncoded,
        encoderInitated: true,
      });
  ...
  MessageDecoderWithScaling = () => {
    ...
    while (
      symbol !== "$" &&
      x < charLimit &&
      decodedSequenceLengthSoFar < msgLength - 1
    ) {
      x += 1;
  ...
  render() {
    ...
    if (this.state.encoderInitated && !this.state.finalScalingInitiated) {
      continueEncodingBtn = (
        <Button btnType="Success" onClick={this.FrequencyInitiation}>
          Continue Encoding
        </Button>
      );
    } else {
      encodeBtn = (
        <Button
          ...
          color="danger"
          onClick={this.FrequencyInitiation}
        >
          Encode MSG
        </Button>
      );
          ...
          <div>
            <div className={classes.decoding}>
              <input
                ...
                placeholder=" please enter the encoded msg"
              ></input>
              <Button
                disabled={!this.state.messageToBeDecoded}
                color="warning"
                onClick={this.MessageDecoder}
              >
                Decode MSG
              </Button>
              {decodeMsgBox}
